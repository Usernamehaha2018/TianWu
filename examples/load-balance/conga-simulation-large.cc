    ...
    for (int i = 0; i < LEAF_COUNT; i++)
    {
        ...
        for (int j = 0; j < SERVER_COUNT; j++)
        {
            ...
            if (runMode == CONGA || runMode == CONGA_FLOW || runMode == CONGA_ECMP)
            {
                ...
                congaRoutingHelper.GetCongaRouting(leaves.Get(i)->GetObject<Ipv4>())->AddRoute(interfaceContainer.GetAddress(1), Ipv4Mask("255.255.255.255"), netDeviceContainer.Get(0)->GetIfIndex());
            ...
            if (runMode == TianWu)
            {
                ...
                tianWuLeaf->SetTianwuParas(tianwu_max, tianwu_min, spineLeafCapacity * LINK_CAPACITY_BASE, tianwu_sched_freq, 1);
            }
    ...
    for (int i = 0; i < LEAF_COUNT; i++)
    {
        ...
        for (int j = 0; j < SPINE_COUNT; j++)
        {
            ...
            for (int l = 0; l < LINK_COUNT; l++)
            {
                ...
                if (isAsymCapacity)
                {
                    spineLeafCapacity = SPINE_LEAF_CAPACITY / 4;
                ...
                if (runMode == TianWu)
                {
                    ...
                    tianWuSpine->SetTianwuParas(tianwu_max, tianwu_min, spineLeafCapacity * LINK_CAPACITY_BASE, tianwu_sched_freq, 0);
                }
            }
        }
    }
